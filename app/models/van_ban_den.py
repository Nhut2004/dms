...
class VanBanDen(Base):
    ...
    # Thông tin ban hành
    organ_id = Column(String(13), nullable=False, comment="Mã định danh cơ quan (OrganId)")
    ...
    # Thông tin xử lý văn bản đến
    arrival_date = Column(Date, nullable=True, comment="Ngày đến (ArrivalDate)")
    arrival_number = Column(Integer, nullable=True, comment="Số đến (ArrivalNumber)")
    to_places = Column(String(1000), nullable=True, comment="Đơn vị hoặc người nhận (ToPlaces)")
    trace_header_list = Column(Text, nullable=True, comment="Ý kiến phân phối, chỉ đạo (TraceHeaderList)")
    due_date = Column(Date, nullable=True, comment="Thời hạn giải quyết (DueDate)")
    
    # Người ký
    signer_position = Column(String(100), nullable=True, comment="Chức vụ người ký")
    signer_full_name = Column(String(50), nullable=True, comment="Họ tên người ký")
    ...
    nguoi_nhap_id = Column(Integer, ForeignKey("tai_khoan_nguoi_dung.id"), nullable=True, comment="Cán bộ văn thư nhập liệu")

    # Quản lý hệ thống
    status = Column(String(50), default="Chờ xử lý", comment="Trạng thái xử lý")
